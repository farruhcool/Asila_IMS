...
class SupplierForm(forms.ModelForm):
    def __init__(self, *args, **kwargs):
        ...
        self.fields['phone'].widget.attrs.update(
            {'class': 'textinput form-control', 'maxlength': '9', 'pattern': '[0-9]{9}', 'title': 'Numbers only'})
        self.fields['email'].widget.attrs.update({'class': 'textinput form-control'})
        self.fields['inn'].widget.attrs.update(
            {'class': 'textinput form-control', 'maxlength': '9', 'pattern': '[0-9]{9}',
             'title': 'INN Format Required'})

    class Meta:
        model = Supplier
        fields = ['name', 'phone', 'address', 'email', 'inn']
        widgets = {
